<template>
  <v-app id="inspire">
    ...
    <v-navigation-drawer v-model="drawer" color="#0468B1">
      <v-sheet color="#0468B1" class="pa-4" style="display: flex; text-align: center; align-items: center;">
        <!-- <v-avatar
            class="mb-4"
            color="grey-darken-1"
            size="64"
    ...
    <v-main>
      <v-card>
        <v-card-text
          style="display: flex; align-items: center; font-size: 30px; text-transform: uppercase; font-family: system-ui; ">
      ...
      <v-container class="py-2 px-6" fluid>
        ...
        <v-main class=" pa-0">
          <v-container class="scrollable ma-0 pa-2">
            <!-- your content here -->
            <table width=100% border=1 cellspacing="0" style="border-collapse: collapse; border: solid 1px #E5E5E5">
              <tr>
                <th>
                  STT
                </th>
              ...
              <tr v-for="(item, i) in data" :key="i"
                ...
                <td style="text-align: center">
                  ...
                  <v-icon color="red"  style="cursor: pointer;" size={1}>mdi-trash-can-outline</v-icon>
                </td>
...
export default {
  ...
  setup() {
    ...
    api_tintuc.get('/filter?page=0&size=20&keyword=&orderFields=&orderTypes=&tinhTrang=&thamChieu_maMuc=&NgayTaoFrom=0&NgayTaoTo=0').then(datas => {
